...
> Résultat : un lot non découpé se voit, le front a un nom et une session part avec un ordre écrit.
...
| # | Task | blocked_by | // OK |
|---|------|------------|-------|
| 1 | RAS-107 clarifier le plan et les preuves (`gate: me`) | — | non |
...
Rien de parallélisable : chaque étage repose sur le précédent. `socle/volets-plan-session` attend RAS-107 et part en parallèle dès qu'il est rendu — c'est la seule arête sortante du sous-lot.

## Preuves attendues

- `window` distingue clos, non découpé, bloqué et lançable.
- Une session refuse de démarrer sans brief.
- Une Run suit l’orchestrateur et ses sous-lots.
- Les tests prouvent qu’un lancement fait réellement évoluer les statuts.

Non couvert : l'agent qui **coupe** un lot non découpé. Ce plan produit la détection ; le découpage lui-même demande un agent Spec lancé avec un périmètre d'écriture.
