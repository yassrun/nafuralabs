...
function SessionView({ groups, selected, running, decisions, spawnPret, busy, onSelect, onOpenPlan, onApprove, onRun }: { groups: Group[]; selected: Group | null; running: Lance[]; decisions: Task[]; spawnPret: boolean; busy: boolean; onSelect: (key: string) => void; onOpenPlan: () => void; onApprove: (id: string) => void; onRun: (group: Group) => void }) {
  ...
  return <><PageHead kicker="Front prêt · maintenant" title="Session d’exécution" subtitle={`${groups.length} sous-lot(s) autorisé(s) par la roadmap.`}>{selected ? <button className="rf-primary" disabled={busy || !spawnPret || isRunning} onClick={() => onRun(selected)}>{isRunning ? "● En cours" : "▶ Lancer ce sous-lot"}</button> : null}</PageHead>
    {!spawnPret ? <div className="rf-notice">Le lancement automatique n’est pas configuré. La session reste lisible, mais aucun agent ne sera démarré.</div> : null}
