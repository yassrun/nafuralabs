#!/usr/bin/env node
/**
 * Regen generated PM views from live task files.
 * Writes: INDEX.tsv, SPRINT.md, BACKLOG.md
 * Never hand-edit those files — run: node pm/regen.mjs
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";

const PM_ROOT = path.dirname(fileURLToPath(import.meta.url));
const ARCHIVE = path.join(PM_ROOT, "backlog_archive");

const STATUS_ORDER = { doing: 0, review: 1, blocked: 2, todo: 3, done: 4 };
const PRIORITY_ORDER = { P0: 0, P1: 1, P2: 2, P3: 3 };
const GLYPH = { todo: "·", doing: "▸", blocked: "✕", review: "◐", done: "✓" };

function walkTasks(dir, out = []) {
  if (!fs.existsSync(dir)) return out;
  for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
    const full = path.join(dir, ent.name);
    if (ent.isDirectory()) {
      if (path.resolve(full) === path.resolve(ARCHIVE)) continue;
      if (ent.name === "tasks") {
        for (const f of fs.readdirSync(full)) {
          if (f.endsWith(".md")) out.push(path.join(full, f));
        }
      } else {
        walkTasks(full, out);
      }
    }
  }
  return out;
}

function parseFrontmatter(raw) {
  if (!raw.startsWith("---\n") && !raw.startsWith("---\r\n")) return null;
  const end = raw.indexOf("\n---", 4);
  if (end < 0) return null;
  const block = raw.slice(4, end).replace(/\r/g, "");
  const body = raw.slice(end + 4).replace(/^\r?\n/, "");
  const fm = {};
  for (const line of block.split("\n")) {
    const m = line.match(/^([a-z_]+):\s*(.*)$/);
    if (!m) continue;
    let v = m[2].trim();
    if (
      (v.startsWith("[") && v.endsWith("]")) ||
      (v.startsWith('"') && v.endsWith('"')) ||
      (v.startsWith("'") && v.endsWith("'"))
    ) {
      // keep raw string for tags etc.; scalar fields are plain
      if (v.startsWith('"') || v.startsWith("'")) v = v.slice(1, -1);
    }
    fm[m[1]] = v;
  }
  const h1 = body.match(/^#\s+(.+)$/m);
  fm._title = h1 ? h1[1].trim() : fm.id || path.basename(raw);
  return fm;
}

function projectFromPath(filePath) {
  const rel = path.relative(PM_ROOT, filePath).replace(/\\/g, "/");
  // nafura/products/<p>/tasks/… | nafura/projects/<p>/tasks/… | nafura/ops/tasks/…
  // saham/tasks/… | personal/tasks/…
  const mProd = rel.match(/^(?:nafura\/)?products\/([^/]+)\//);
  if (mProd) return { key: mProd[1], context: "nafura", type: "product" };
  const mProj = rel.match(/^(?:nafura\/)?projects\/([^/]+)\//);
  if (mProj) return { key: mProj[1], context: "nafura", type: "project" };
  const mProto = rel.match(/^(?:nafura\/)?prototypes\/([^/]+)\//);
  if (mProto) return { key: mProto[1], context: "nafura", type: "prototype" };
  if (rel.startsWith("nafura/ops/")) return { key: "ops", context: "nafura", type: "ops" };
  if (rel.startsWith("saham/")) return { key: "saham", context: "saham", type: "engagement" };
  if (rel.startsWith("personal/")) return { key: "personal", context: "personal", type: "personal" };
  return { key: "misc", context: "nafura", type: "misc" };
}

function shortTitle(title, kind) {
  let t = title
    .replace(/^Feature\s+[—–-]\s+/i, "")
    .replace(/^Bug\s+[—–-]\s+/i, "Bug — ")
    .replace(/\s+/g, " ")
    .trim();
  if (kind === "feature" && !title.toLowerCase().includes("feature")) {
    /* keep cleaned */
  }
  if (t.length > 48) t = t.slice(0, 45) + "…";
  return t;
}

function loadTasks() {
  const files = walkTasks(PM_ROOT);
  const tasks = [];
  for (const file of files) {
    const fm = parseFrontmatter(fs.readFileSync(file, "utf8"));
    if (!fm?.id) continue;
    const proj = projectFromPath(file);
    tasks.push({
      id: fm.id,
      status: fm.status || "todo",
      priority: fm.priority || "P3",
      context: fm.context || proj.context,
      assignee: fm.assignee || "",
      gate: fm.gate || "",
      kind: fm.kind || "task",
      sprint: fm.sprint || "",
      parent: fm.parent || "",
      feature: fm.feature || "",
      title: shortTitle(fm._title, fm.kind),
      titleFull: fm._title,
      project: proj.key,
      file,
    });
  }
  return tasks;
}

function sortTasks(a, b) {
  const sa = STATUS_ORDER[a.status] ?? 9;
  const sb = STATUS_ORDER[b.status] ?? 9;
  if (sa !== sb) return sa - sb;
  const pa = PRIORITY_ORDER[a.priority] ?? 9;
  const pb = PRIORITY_ORDER[b.priority] ?? 9;
  if (pa !== pb) return pa - pb;
  const fa = a.feature || a.id;
  const fb = b.feature || b.id;
  if (fa !== fb) return fa < fb ? -1 : 1;
  // umbrella (feature, no parent) before children
  const ua = a.kind === "feature" && !a.parent ? 0 : 1;
  const ub = b.kind === "feature" && !b.parent ? 0 : 1;
  if (ua !== ub) return ua - ub;
  return a.id < b.id ? -1 : a.id > b.id ? 1 : 0;
}

function isoWeekInfo(d = new Date()) {
  // ISO week: Thursday-based
  const date = new Date(Date.UTC(d.getFullYear(), d.getMonth(), d.getDate()));
  const day = date.getUTCDay() || 7;
  date.setUTCDate(date.getUTCDate() + 4 - day);
  const yearStart = new Date(Date.UTC(date.getUTCFullYear(), 0, 1));
  const week = Math.ceil(((date - yearStart) / 86400000 + 1) / 7);
  const year = date.getUTCFullYear();
  // Monday of this ISO week
  const monday = new Date(d);
  const dow = monday.getDay() || 7;
  monday.setDate(monday.getDate() - dow + 1);
  monday.setHours(0, 0, 0, 0);
  const sunday = new Date(monday);
  sunday.setDate(monday.getDate() + 6);
  const fmt = (x) =>
    `${String(x.getDate()).padStart(2, "0")}/${String(x.getMonth() + 1).padStart(2, "0")}`;
  return {
    id: `${year}-W${week}`,
    label: `lun ${fmt(monday)} → dim ${fmt(sunday)}`,
  };
}

function writeIndex(tasks) {
  const sorted = [...tasks].sort(sortTasks);
  const header =
    "id\tstatus\tpriority\tcontext\tassignee\tgate\tkind\tsprint\tparent\tfeature\ttitle";
  const rows = sorted.map((t) =>
    [
      t.id,
      t.status,
      t.priority,
      t.context,
      t.assignee,
      t.gate,
      t.kind,
      t.sprint,
      t.parent,
      t.feature,
      t.title,
    ].join("\t")
  );
  fs.writeFileSync(
    path.join(PM_ROOT, "INDEX.tsv"),
    [header, ...rows].join("\n") + "\n",
    "utf8"
  );
  return sorted.length;
}

function padId(id) {
  return id.padEnd(7);
}

function writeSprint(tasks) {
  const { id, label } = isoWeekInfo();
  const committed = tasks.filter((t) => t.sprint === id).sort(sortTasks);
  const readyP1 = tasks
    .filter((t) => !t.sprint && t.kind !== "feature" && t.priority === "P1" && t.status === "todo")
    .sort(sortTasks);

  const lines = [`# SPRINT ${id}                         ${label}`, ""];

  if (committed.length === 0) {
    lines.push(`  (aucune tâche commitée — Commit = poser \`sprint: ${id}\`)`);
    lines.push("");
    if (readyP1.length) {
      lines.push("  tasks P1 prêtes (hors features) :");
      for (const t of readyP1) {
        const who = t.assignee ? `[${t.assignee}]` : "";
        const gate = t.gate ? `gate:${t.gate}` : "";
        lines.push(
          `  ${GLYPH[t.status] || "·"} ${padId(t.id)}  ${t.title.padEnd(32).slice(0, 32)}  ${who.padEnd(8)}  ${gate}`
        );
      }
      lines.push("");
    }
  } else {
    for (const t of committed) {
      const who = t.assignee ? `[${t.assignee}]` : "";
      const gate = t.gate ? `gate:${t.gate}` : "";
      lines.push(
        `${GLYPH[t.status] || "·"} ${padId(t.id)}  ${t.title.padEnd(32).slice(0, 32)}  ${t.priority}  ${who.padEnd(8)}  ${gate}`
      );
    }
    lines.push("");
  }

  const counts = {
    committed: committed.length,
    doing: committed.filter((t) => t.status === "doing").length,
    review: committed.filter((t) => t.status === "review").length,
    blocked: committed.filter((t) => t.status === "blocked").length,
  };
  lines.push("  ────────────────────────────────────────────────────────────────");
  lines.push(
    `  committed  ${counts.committed}   doing ${counts.doing}   review ${counts.review}   blocked ${counts.blocked}`
  );
  lines.push("  ────────────────────────────────────────────────────────────────");
  lines.push("");

  fs.writeFileSync(path.join(PM_ROOT, "SPRINT.md"), lines.join("\n"), "utf8");
}

/** Human BACKLOG: keep feature clusters together (unlike INDEX priority-first). */
function sortBacklogCluster(a, b) {
  const ua = a.kind === "feature" && !a.parent ? 0 : 1;
  const ub = b.kind === "feature" && !b.parent ? 0 : 1;
  if (ua !== ub) return ua - ub;
  const sa = STATUS_ORDER[a.status] ?? 9;
  const sb = STATUS_ORDER[b.status] ?? 9;
  if (sa !== sb) return sa - sb;
  const pa = PRIORITY_ORDER[a.priority] ?? 9;
  const pb = PRIORITY_ORDER[b.priority] ?? 9;
  if (pa !== pb) return pa - pb;
  return a.id < b.id ? -1 : a.id > b.id ? 1 : 0;
}

function clusterKey(t) {
  return t.feature || `_${t.id}`;
}

function clusterPriority(list) {
  const umbrella = list.find((t) => t.kind === "feature" && !t.parent);
  if (umbrella) return PRIORITY_ORDER[umbrella.priority] ?? 9;
  return Math.min(...list.map((t) => PRIORITY_ORDER[t.priority] ?? 9));
}

function writeBacklog(tasks) {
  const byProject = new Map();
  for (const t of tasks) {
    if (!byProject.has(t.project)) byProject.set(t.project, []);
    byProject.get(t.project).push(t);
  }

  const projectOrder = [...byProject.keys()].sort((a, b) => {
    const na = byProject.get(a).length;
    const nb = byProject.get(b).length;
    if (na !== nb) return nb - na;
    return a < b ? -1 : 1;
  });

  const lines = [
    "# BACKLOG (généré — ne pas éditer)",
    "",
    "> Vue humaine live. Source = `…/tasks/*.md`. Regen : `node pm/regen.mjs` / `node pm/t.mjs index`.",
    "> Agents : lire `INDEX.tsv` d'abord. Inbox brute : `inbox.md` (hors cette vue).",
    "",
  ];

  for (const proj of projectOrder) {
    const list = byProject.get(proj);
    const clusters = new Map();
    for (const t of list) {
      const k = clusterKey(t);
      if (!clusters.has(k)) clusters.set(k, []);
      clusters.get(k).push(t);
    }
    const clusterOrder = [...clusters.keys()].sort((a, b) => {
      const pa = clusterPriority(clusters.get(a));
      const pb = clusterPriority(clusters.get(b));
      if (pa !== pb) return pa - pb;
      return a < b ? -1 : 1;
    });

    lines.push(`## ${proj}`);
    lines.push("");
    for (const ck of clusterOrder) {
      const members = clusters.get(ck).sort(sortBacklogCluster);
      for (const t of members) {
        const g = GLYPH[t.status] || "·";
        const kindOrStatus = t.kind === "feature" ? "feature" : t.status;
        const bullet = t.parent ? "  -" : "-";
        lines.push(
          `${bullet} ${g} \`${padId(t.id).trim()}\` ${kindOrStatus} — ${t.title}`
        );
      }
    }
    lines.push("");
  }

  lines.push("---");
  lines.push("");
  lines.push(`**${tasks.length} live · ${projectOrder.length} projets**`);
  lines.push("");

  fs.writeFileSync(path.join(PM_ROOT, "BACKLOG.md"), lines.join("\n"), "utf8");
}

export function regen() {
  const tasks = loadTasks();
  const n = writeIndex(tasks);
  writeSprint(tasks);
  writeBacklog(tasks);
  return { tasks: n, projects: new Set(tasks.map((t) => t.project)).size };
}

const isDirect =
  process.argv[1] &&
  path.resolve(process.argv[1]) === fileURLToPath(import.meta.url);

if (isDirect) {
  const r = regen();
  console.log(
    `regen ok — ${r.tasks} tasks, ${r.projects} projects → INDEX.tsv SPRINT.md BACKLOG.md`
  );
}
