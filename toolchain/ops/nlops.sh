...
build_usage_ops_images() {
  local tag
  tag="$(image_tag_for_env)"
  local backend_img web_img lifecycle_img
  backend_img="$(image_ref usage-ops-backend "$tag")"
  web_img="$(image_ref usage-ops-web "$tag")"
  lifecycle_img="$(image_ref nafura-lifecycle "$tag")"

  echo "Building backend -> $backend_img"
  (cd "$ROOT" && "$GRADLEW" :usage-ops:app:bootJar --no-daemon)
  docker build -t "$backend_img" -f "$ROOT/products/usage-ops/Dockerfile.jar" \
    "$ROOT/products/usage-ops/backend/app/build/libs"

  echo "Building frontend -> $web_img"
  docker build -t "$web_img" -f "$ROOT/products/usage-ops/Dockerfile.web" "$ROOT"

  echo "Building lifecycle -> $lifecycle_img"
  (cd "$ROOT" && "$GRADLEW" :tools:lifecycle:collectMigrations -PappId=usage-ops --no-daemon)
  docker build -t "$lifecycle_img" -f "$ROOT/tools/lifecycle/Dockerfile" "$ROOT/tools/lifecycle"
