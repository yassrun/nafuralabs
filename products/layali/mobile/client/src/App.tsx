...
import type { AccessMode, BookingDraft, Occasion, VenueDetail } from '@shared/prototypeData'
import { PRO_APP_URL } from '@shared/env'
import './App.css'
...
function App() {
  ...
  const [state, setState] = useState<AppState>({
    ...
    selectedBookingId: initialRoute.screen === 'booking-detail' ? initialRoute.id : undefined,
    booking: mockBooking,
  ...
  const applyNavigation = (prev: AppState, screen: Screen, id?: string): AppState => {
    ...
    if (id) {
      ...
      if (screen === 'booking-detail') next.selectedBookingId = id
    }
  ...
  const isDashboardTabActive = state.currentScreen === 'home'
      ...
      <IonPage>
        ...
        {shouldShowBottomNav && (
          <nav className="bottom-nav" aria-label="Navigation principale">
            ...
            <button type="button" className={state.currentScreen === 'customer-profile' ? 'is-active' : ''} onClick={() => navigate('customer-profile')}>
              <span aria-hidden="true">◉</span>
              <span>Profil</span>
            </button>
          </nav>
...
function HomeScreen({
  ...
  return (
    <main className="feed-root">
      ...
      <footer className="client-footer-pro">
        <button
          type="button"
          className="client-footer-pro__link"
          onClick={() => globalThis.open(PRO_APP_URL, '_blank')}
        >
        ...
        {!otpSent ? (
          <>
            ...
            <section className="sticky-cta sticky-cta--stacked">
              ...
              <button type="button" className="link-btn" onClick={() => navigate('home')}>Retour</button>
            </section>
          </>
        ) : (
          <>
