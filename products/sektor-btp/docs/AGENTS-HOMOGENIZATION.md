...
| Source de vérité couleur | `app/styles/brand-sektor.scss` |
...
> ✅ **STATUT (mis à jour 2026-06-17) : la passe d'homogénisation de masse est FAITE.** Mesuré sur `app` :
...
- **Détection** des bleus en dur qui ne suivront PAS le token (à corriger en WP4) :
  ```bash
  grep -rEn "#2563eb|#1d4ed8|#3b82f6|#1e40af|#1e3a8a|#93c5fd" app
  ```
...
- **Détection** :
  ```bash
  grep -rEn "<button" app/pages/<feature> --include=*.html --include=*.ts
  ```
...
- **Détection** : `grep -rEn "mat-form-field|mat-select|matInput" app/pages/<feature>`
...
- **Détection** : `grep -rEn "#[0-9a-fA-F]{3,6}\b" app/pages/<feature> --include=*.html --include=*.scss --include=*.ts`
...
- **Détection** : `grep -rEn "<mat-icon" app/pages/<feature>`
...
- **Détection** : `grep -rEn "style=\"" app/pages/<feature> --include=*.html`
...
1. Choisir **une** feature non traitée dans `app/pages/` (tenir un tableau d'avancement en tête de PR).
...
- `grep -rEn "<button|mat-form-field|mat-select|mat-icon|#[0-9a-fA-F]{6}" app` → **0** (hors lib anatomy).
