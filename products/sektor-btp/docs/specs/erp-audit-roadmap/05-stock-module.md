...
> **Dépendances** : `01-foundations`. Réutilise `app/pages/inventory/` (déjà à 70% — sert de référence pattern).
...
```
app/pages/inventory/                          # base existante (70% complet)
app/inventory/inventory.routes.ts             # routing module
app/inventory/mock/                           # mock service
...
```ts
// app/pages/stock/articles/models/index.ts
...
**Fichier à créer** : `app/pages/stock/stock.routes.ts`
...
**Et dans** : `app/inventory/inventory.routes.ts` (ou créer `stock.routes.ts` parent) — wirer dans le routeur principal ERP.
