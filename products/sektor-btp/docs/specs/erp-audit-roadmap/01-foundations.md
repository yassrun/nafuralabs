...
app/chantiers/mock/chantiers-mock.service.ts  # mock seed actuel CH-2026
app/chantiers/mock/seeds.ts                   # seed unique attendu
app/pages/inventory/catalogue/materiel/config/detail/fields.ts  # clés i18n
...
**État actuel** :
- `app/chantiers/mock/seeds.ts` → `SEED_CHANTIERS` (canon, 6 chantiers `CH-2026-001..006`)
- Mais : `Mes chantiers` page legacy semble avoir d'autres seeds (`CH-2025-XXX`, `PROJ-2024-XXX`) — **rechercher et purger**.

**Fichiers à auditer** :

```bash
grep -rn "CH-2025\|CH-2024\|PROJ-2024\|PROJ-2025" app/
grep -rn "SEED_\|MOCK_\|seed[A-Z]" app/ | grep -v "node_modules"
```

**Action** :

1. Lister tous les datasets mock chantiers détectés.
2. Pour chaque dataset détecté : remplacer son contenu par un import depuis `app/chantiers/mock/seeds.ts`.
...
**Acceptance criteria** :
- [ ] `grep -rn "CH-2025\|PROJ-2024\|PROJ-2025" app/` retourne 0 résultats hors tests historiques explicitement nommés
...
**Fichier source des clés** : `app/pages/inventory/catalogue/materiel/config/detail/fields.ts` (lignes 12, 21, 29, 41, 50…)
...
**Fichier** : `app/pages/dashboard/dashboard.page.ts`
