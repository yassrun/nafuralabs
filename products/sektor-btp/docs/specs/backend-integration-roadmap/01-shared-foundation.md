...
```
web/app/pages/inventory/catalogue/items/services/item-api.service.ts            ← déjà pure HTTP
web/app/pages/inventory/catalogue/item-prices/services/item-price-api.service.ts ← déjà pure HTTP
web/app/pages/inventory/configuration/item-categories/services/*.ts              ← déjà pure HTTP
web/app/pages/inventory/configuration/item-types/services/*.ts                   ← déjà pure HTTP
web/app/pages/inventory/configuration/unit-of-measures/services/*.ts             ← déjà pure HTTP
web/app/pages/inventory/configuration/uo-mcategories/services/*.ts               ← déjà pure HTTP
web/app/pages/inventory/mouvements/inventory-txes/services/inventory-tx-api.service.ts ← déjà pure HTTP
web/app/pages/inventory/mouvements/inventory-tx-lines/services/inventory-tx-line-api.service.ts ← déjà pure HTTP
web/app/pages/inventory/suivi/stock-balances/services/stock-balance-api.service.ts     ← déjà pure HTTP
web/app/pages/finance/configuration/currencies/services/currency-api.service.ts        ← déjà pure HTTP
web/app/pages/finance/configuration/exchange-rates/services/exchange-rate-api.service.ts ← déjà pure HTTP
web/app/pages/finance/configuration/payment-terms/services/payment-term-api.service.ts ← déjà pure HTTP
```
...
> Les attributs ICE / IF / RC / Patente / CNSS / AMO sont des **champs sur `Partner`**, pas une entité séparée. Validation algorithmique (cf. `web/app/shared/validators/`) reportée côté backend.
...
Charger les fournisseurs de `web/app/achats/mock/achats-mock.service.ts` et les clients de `web/app/ventes/mock/ventes-mock.service.ts` pour pouvoir comparer le frontend mock vs réel.
...
- [ ] Validation `IceValidator` côté backend (15 chiffres + clé) reproduisant `web/app/shared/validators/`.
...
1. Auditer `web/app/**/*.facade.ts` qui font des appels d'autocomplete : recenser tous les patterns "charger les fournisseurs / clients / articles / dépôts" et confirmer qu'ils peuvent tous être remplacés par `getLookup()` du `FeatureApiService`.
...
```bash
grep -r "MockService" web/app/pages/inventory/catalogue/ \
                     web/app/pages/finance/configuration/ \
                     2>/dev/null
# (vide attendu)
```
