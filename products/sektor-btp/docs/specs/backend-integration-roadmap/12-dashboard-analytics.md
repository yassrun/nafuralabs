...
grep -rE "inject\((AchatsMockService|VentesMockService|ChantiersMockService|RhMockService|HseMockService|InventoryMockService|FinanceComptabiliteMockService))\)" \
  web/app/pages/dashboard/ \
  web/app/pages/analytics/ \
  web/app/pages/pilotage/ \
  web/app/pages/pilotage-analyses/ \
  web/app/shell/ \
  2>/dev/null
# (vide attendu)
```
...
- [ ] **Migration complète :** `grep MockService` sur tout `web/app/` (hors specs) → vide
