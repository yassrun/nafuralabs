...
- La logique métier critique (3-way matching, scoring AO, workflow DA→AO→BC→FF) est **codée côté frontend** dans `web/app/achats/services/matching.service.ts` → à déplacer côté backend.
...
**Scoring AO côté backend :** reprendre la logique de `web/app/achats/services/matching.service.ts` (composantes prix /50, délai /15, qualité /15, historique /10, art187 /10).
...
- `web/app/achats/services/matching.service.ts` (reste comme orchestrateur frontend pur, sans `inject(AchatsMockService)`).
...
grep -rE "inject\(AchatsMockService\)" \
  web/app/achats/ \
  web/app/pages/achats/ \
  web/app/pages/finance/factures-fournisseurs/ \
  2>/dev/null
# (vide attendu)
```
