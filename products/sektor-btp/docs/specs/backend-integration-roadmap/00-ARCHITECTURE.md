...
2. **Même découpe que le frontend** : un dossier `web/app/pages/<module>/` ⇄ un dossier `backend/domains/<module>/`. Pas de "super-module" backend qui regroupe achats + ventes.
...
Recherche grep finale par module : `grep -r "MockService" web/app/<module>/ web/app/pages/<module>/` doit retourner **zéro résultat** avant de marquer la tâche `[x]`.

### 4.4 Mock service : quarantaine puis suppression

Le fichier `web/app/<module>/mock/<module>-mock.service.ts` est :
