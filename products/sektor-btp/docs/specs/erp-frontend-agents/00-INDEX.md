...
> Source de vérité nav : `web/app/shell/erp-nav.generated.ts`.
> Module pilote : `web/app/pages/inventory/` (~70% complet, sert de référence pattern).
...
- Composants partagés : `web/app/inventory/components/` (réutilisables).
- Modèles partagés inventory : `web/app/inventory/models/index.ts`.
- Routes ERP : `web/app/routes/erp.routes.generated.ts` (généré) + `inventory.routes.ts` (custom hand-written, modèle à suivre pour les modules non générés).
- Nav source : `web/app/shell/erp-nav.generated.ts`.
