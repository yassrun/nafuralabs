...
> Pattern de référence : `web/app/pages/inventory/catalogue/articles/` (entité simple)
> et `web/app/pages/inventory/mouvements/receptions/` (entité avec workflow + lignes).
...
1. Créer `web/app/<module>/<module>.routes.ts` (hand-written, suit `inventory.routes.ts`).
...
Pattern : `web/app/pages/inventory/mouvements/receptions/config/detail/`.
