...
import { SHORTCUTS_CONFIG } from './shortcuts-config.token';
...
const BASE_SHORTCUTS: ShortcutDef[] = [
...
export class ShortcutsService {
  ...
  private readonly shortcutsConfig = inject(SHORTCUTS_CONFIG);
  private gMode = false;
  private gTimer: ReturnType<typeof setTimeout> | null = null;

  get shortcuts(): ShortcutDef[] {
    return [...BASE_SHORTCUTS, ...this.shortcutsConfig.extraShortcuts];
  }
  ...
  handle(event: KeyboardEvent, callbacks: {
    ...
    // g <key> → goto (product-specific map via SHORTCUTS_CONFIG)
    if (this.gMode) {
      this.clearGMode();
      const route = this.shortcutsConfig.gotoMap[event.key.toLowerCase()];
      if (route) {
