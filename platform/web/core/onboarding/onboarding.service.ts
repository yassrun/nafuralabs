...
import { ONBOARDING_CONFIG } from './onboarding-config.token';
...
export class OnboardingService {
  ...
  private readonly onboardingConfig = inject(ONBOARDING_CONFIG);
  ...
  get tours(): Tour[] {
    return this.onboardingConfig.tours;
  }

  /** Start a tour by id. */
  start(tourId: string): void {
    const tour = this.onboardingConfig.tours.find((t) => t.id === tourId);
    if (!tour) return;
  ...
  resolveTourForUrl(path: string): string | null {
    for (const entry of this.onboardingConfig.routeTourMap) {
