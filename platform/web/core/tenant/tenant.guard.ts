...
export const tenantRequiredGuard: CanActivateFn = () => {
  ...
  if (!inject(APPLICATION_RUNTIME_CONFIG).requiresTenant) {
    return true;
  }
...
export const tenantSelectionGuard: CanActivateFn = () => {
  ...
  const runtime = inject(APPLICATION_RUNTIME_CONFIG);
  if (runtime.requiresTenant) {
    return true;
  }

  const defaultRoute = runtime.defaultRoute || 'feature-unavailable/unknown';
