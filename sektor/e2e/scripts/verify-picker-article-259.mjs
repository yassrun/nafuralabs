/**
 * Preuve SEKTOR-259 — API recherche items serveur (picker).
 * Run: node sektor/e2e/scripts/verify-picker-article-259.mjs
 *
 * Surface dédiée GET /api/v1/items/search — ne pas casser GET /api/v1/items.
 * Couvre AC-1…AC-6 côté API.
 */
...
async function main() {
  ...
  // AC-1 — sans q≥2 et sans filtre → page vide
  ...
  assert(
    Array.isArray(empty.body?.content),
    `search sans q: content[] attendu, got ${JSON.stringify(empty.body)?.slice(0, 180)}`,
  );
  assert(
    empty.body.content.length === 0 && empty.body.totalElements === 0,
    `AC-1: search sans q/filtre doit être vide, got ${empty.body.content.length}/${empty.body.totalElements}`,
  );
  ...
  assert(
    contentOf(shortQ.body).length === 0 && totalOf(shortQ.body) === 0,
    'q<2 sans filtre → page vide 200',
  );

  const suffix = Date.now().toString(36);
  const exactCode = `ART-P259-${suffix}`.slice(0, 20);
  const skuOnly = `SKU-P259-${suffix}`;

  const cats = await json(
    await fetch(`${API_BASE}/api/v1/item-categories?page=0&size=200`, { headers: h }),
  );
  ...
  const mo = await createItem({
    name: `Coffreur picker 259 ${suffix}`,
    nature: 'MAIN_DOEUVRE',
    isActive: true,
  ...
  const inactive = await createItem({
    name: `Inactif picker 259 ${suffix}`,
    nature: 'MATIERE',
    isActive: false,
    unitOfMeasureId: uomId,
  });

  // AC-2 — code exact en tête ; searchFields = code + name
  const exactSearch = await json(
    await fetch(`${API_BASE}/api/v1/items/search?q=${encodeURIComponent(exactCode)}`, {
      headers: h,
    }),
  );
  ...
  // AC-6 — actifs seulement par défaut
  assert(
    exactHits.every((r) => r.isActive !== false),
    'AC-6: hits actifs seulement par défaut',
  );
  assert(!exactHits.some((r) => r.id === inactive.id), 'inactif absent du search défaut');

  const skuSearch = await json(
    await fetch(`${API_BASE}/api/v1/items/search?q=${encodeURIComponent(skuOnly)}`, {
      headers: h,
    }),
  );
  ...
  // AC-3 / AC-4 — nature seule déclenche ; filtre serveur
  const natureHits = await json(
    await fetch(`${API_BASE}/api/v1/items/search?nature=MAIN_DOEUVRE`, { headers: h }),
  );
  ...
  const lot = await json(
    await fetch(`${API_BASE}/api/v1/items/search?usageLot=GROS_OEUVRE`, { headers: h }),
  );
  assert(lot.status === 200, `search usageLot ${lot.status} ${lot.text}`);
  assert(contentOf(lot.body).some((r) => r.id === exact.id), 'filtre usageLot serveur');
  ...
  const ids0 = contentOf(page0.body)
    .map((r) => r.id)
    .join(',');
  const ids1 = contentOf(page1.body)
    .map((r) => r.id)
    .join(',');
  assert(ids1.length > 0 && ids0 !== ids1, `AC-5: page suivante distincte (${ids0} vs ${ids1})`);

  const listingAfter = await json(
    await fetch(`${API_BASE}/api/v1/items?page=0&size=5`, { headers: h }),
  );
  assert(contentOf(listingAfter.body).length > 0, 'listing générique intact après search');

  console.log(
    `PASS SEKTOR-259 — search vide=${empty.body.totalElements} exact=${exactHits[0].code} nature=${natureRows.length} pageTotal=${totalOf(page0.body)}`,
  );
}
