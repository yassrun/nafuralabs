...
public class DpuService {
    ...
    private String mapOuvrageTypeToDpu(String type) {
        return CatalogNatureMapping.toDpuTypeFromOuvrage(type);
    }
    ...
    private ComposantDpu buildComposant(
        ...
        if (ref.type() == ReferenceType.ITEM
            ...
            CatalogPriceSnapshot resolu = gelPrixService.resoudre(ref.itemId());
            if (resolu != null && resolu.unitPrice() != null) {
                gelPrixService.appliquerGel(builder, resolu);
                prixUnitaire = resolu.unitPrice();
                sourcePrix = resolu.priceSource();
                if (CatalogPriceSource.CONSULTE.equals(resolu.priceSource())) {
    ...
    public PrixDpu refreshPrices(UUID dpuId) {
        ...
        for (ComposantDpu composant : entity.getComposants()) {
            ...
            CatalogPriceSnapshot resolu = gelPrixService.resoudre(composant.getItemId());
            if (resolu == null || resolu.unitPrice() == null) {
                continue;
            }
