...
import ma.nafura.socle.onboarding.api.dto.OnboardingDtos.ApplyPresetRequest;
...
public class TenantPresetOrchestratorService {
    ...
    public ApplyPresetResponse applyPreset(UUID tenantId, ApplyPresetRequest rawRequest) {
        ...
        boolean skipChart = financeOnboardingPort.hasChart(tenantId);
        boolean skipArticles = catalogueOnboardingPort.hasArticles(tenantId);
        ...
        try {
            ...
            catalogueOnboardingPort.seedReferenceData(tenantId);
            steps.add("referenceData");

            if (request.forceReset() || !skipChart) {
                financeOnboardingPort.resetChartToSeed();
            ...
            if (!skipArticles) {
                catalogueOnboardingPort.seedArticles(tenantId, request.secteur());
                steps.add("articles");
            } else {
