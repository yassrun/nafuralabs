...
import { onboardingCompleteGuard } from '@app/socle/onboarding/guards/onboarding-complete.guard';
...
import { ONBOARDING_V2_ROUTES } from '@app/socle/onboarding/onboarding.routes';
import { APP_SHELL_CONFIGS } from '@app/socle/config/shell.config';
import {
  resolveApplicationNavigation,
  resolveApplicationZoneConfig,
} from '@app/socle/config/navigation';
    ...
    children: [
      {
        path: '',
        loadComponent: () =>
          import('@app/socle/invitations/pages/invite-accept.page').then(
...

  {
    ...
    children: [
      ...
      },
      {
        ...
        loadChildren: async () => {
          const [platform, app] = await Promise.all([
            ...
            import('@app/socle/administration-routes'),
          ]);
      ...
      },
      {
        ...
        data: { breadcrumb: 'Administration' },
        loadComponent: () =>
          import('@app/socle/administration/hub/admin-hub.page').then(
            (m) => m.AdminHubPage
          ),
      },
      ...APPLICATION_ROUTES,
