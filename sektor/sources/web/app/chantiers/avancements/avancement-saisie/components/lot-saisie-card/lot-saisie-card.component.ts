...
import type { SaisieLineViewModel } from '../../../models';
import { PhotoUploaderComponent } from '../photo-uploader/photo-uploader.component';

@Component({
  selector: 'app-lot-saisie-card',
  standalone: true,
  imports: [FormsModule, TranslateModule, ButtonComponent, PhotoUploaderComponent],
...
export class LotSaisieCardComponent {
  readonly model = input.required<SaisieLineViewModel>();

  readonly quantityChanged = output<number | null>();
  readonly notesChanged = output<string>();
  readonly photosChanged = output<SaisieLineViewModel['photos']>();
  readonly removed = output<void>();

  readonly isForfait = computed(() => {
    const u = this.model().unite?.toUpperCase();
  ...
  onPercentInput(value: string | number | null): void {
    ...
    const lotQte = this.model().quantiteReference || 1;
