...
## Type Extension — `@platform/lib/anatomy/types`
...
| File | Change |
|---|---|
| `@platform/lib/anatomy/types` | Extend `DetailToolbarAction` with `permission`, `disabled`, `disabledTooltip`, `showInModes`, `confirm` |
| `@platform/lib/anatomy/components/organisms/entity-detail` | Render left/right toolbar zones; auto-render status machine in right zone; remove `nfActionCenter` slot dependency |
