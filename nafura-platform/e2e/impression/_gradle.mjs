/**
 * Lance les tests JUnit du rendu impression (jar doc-manager), une fois par run.
 * Run : node --test nafura-platform/e2e/impression/*.test.mjs
 */
...
const RESULTS = path.join(
  BACKEND,
  "impression/build/test-results/test"
);
const LOCK = path.join(RESULTS, "..", "impression-e2e.lock");
...
function runGradle() {
  const args = [":platform:impression:test"];
...
export function runImpressionJunit() {
