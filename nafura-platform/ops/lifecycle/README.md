...
```bash
# From nafura-platform/ops/lifecycle/
../../sources/backend/gradlew -p . collectMigrations

# App-scoped collection (Gradle deps from <app>/sources/backend/app/build.gradle)
../../sources/backend/gradlew -p . collectMigrations -PappId=sektor-btp

# Alias erp → sektor-btp
../../sources/backend/gradlew -p . collectMigrations -PappId=erp

# Optional: print collected files
../../sources/backend/gradlew -p . listMigrations
```
...
```bash
# From repo root
docker build -t nafura-lifecycle:dev -f nafura-platform/ops/lifecycle/Dockerfile nafura-platform/ops/lifecycle
```
