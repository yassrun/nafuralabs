...
class StatelessExtractionServiceTest {
    ...
    @Test
    void imageWithoutGridSkipsHeuristicAndKeepsModelPath() throws Exception {
        byte[] png = new byte[] {(byte) 0x89, 0x50, 0x4E, 0x47};
        LlmResponse llmResponse = planLlm("{\"items\":[]}");
        when(llmService.callLlm(any(), any()))
                .thenReturn(CompletableFuture.completedFuture(llmResponse));

        StatelessExtractionResponse response = service.process(
                png, "scan.png", "image/png", ITEMS_SCHEMA, null, null, "tenant-a");

        ArgumentCaptor<LlmCallContext> context = ArgumentCaptor.forClass(LlmCallContext.class);
        verify(llmService, times(1)).callLlm(any(), context.capture());
        assertThat(context.getValue().getActionKey()).isEqualTo("extract");
        assertThat(response.requestId()).isNotEqualTo("plan:heuristic");
    }
