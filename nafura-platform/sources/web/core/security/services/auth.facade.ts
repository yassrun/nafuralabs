...
export class AuthFacade {
  ...
  async attachOnboardingTenant(
    ...
    // Keep the existing backend onboarding JWT (e.g. from signup).
  ...
  private async tryRestoreDevPersistedSession(): Promise<boolean> {
    ...
    const payload = this.tokenService.decodeAccessToken(session.tokens.accessToken);
    if (!payload?.sub || this.tokenService.isMockToken(session.tokens.accessToken)) {
  ...
  /**
   * Former in-browser mock JWT login. Removed — use Cursor QA or Keycloak.
   */
  private async bypassLoginForDev(): Promise<void> {
    throw new Error(
      'Mock JWT login removed. Use cursorAuthAutoLogin (Mode B) or Keycloak.',
    );
  }
